...
Generally there are 3 steps in setting up any R session:
  1. Choose the packages you are going to use and tell R to equip them by the `library()` command. 
    + If you are using new packages, you will first need to install them, but remember to remove or comment the `install.packages()` line after you have done this. 
...
```{r}
country_ind %>% 
  filter(year == 2017) %>% # filtering to only include 2017 (getting the right rows)
  group_by(region) %>% # the category im interested in
  summarize(mean_le = mean(life_expectancy)) # summary measure im interested in 
```
...
gss %>% 
  ...
  # calculating two summary measures
  summarize(mean_age = mean(age), 
            sd_age = sd(age)) %>% 
  arrange(mean_age) # ordering the results be mean age in ascending order 
```
...
```{r}
gss %>% 
  filter(!is.na(marital_status)) %>% # filter to only include the rows that have a non-missing marital status
  group_by(marital_status) %>% 
  summarize(mean_age = mean(age), 
            sd_age = sd(age)) %>% 
  arrange(mean_age)
```
What about `drop_na()`? This function drops all rows that have ANY NA's in ANY COLUMNS
...
BUT Monica just learned that you can specify which variables you want to drop NAs from

```{r}
gss %>% 
  drop_na(marital_status) %>% # this is equivalent to filter(!is.na(marital_status))
  summarize(mean_age = mean(age), 
            sd_age = sd(age)) %>% 
  arrange(mean_age)
```
...
Side note: na.rm = TRUE happens within functions like mean or sd or median , so it's an argument
...
is.na() is a function itself. So a longer version of the above calculation would be

```{r}
gss %>% 
  filter(!is.na(age_at_first_birth)) %>%  # testing to see which ones are non-missing
  summarize(mean_age_fb = mean(age_at_first_birth))
```
...
gss %>% 
  group_by(province) %>% 
  tally() # a column (by default it's called 'n') showing the number of observations
```
equivalent:
```{r}
gss %>% 
...
gss %>% 
  group_by(province) %>% 
  tally() %>% # are as above, getting the counts by group (province)
  # make a new column (mutate) called prop, equal to the number in that group, divided through by the sum/total
  mutate(prop = n / sum(n))
```
equivalent
...
Imagine we wanted to have the proportion of total respondents by province in our original GSS dataset. Using left_join

```{r}
gss_with_prov_props <- gss %>% 
  left_join(province_prop, by = "province")
```
...
First thing we want to do is work out who has missing values

```{r}
gss <- gss %>% 
  mutate(is_afm_miss = is.na(age_at_first_marriage)) # testing to see whether missing

# have a look
gss %>% 
  select(age_at_first_marriage, is_afm_miss)

# use new column to calculate proportion missing
# note: n() = total number of rows

gss %>% 
  summarize(prop_miss = sum(is_afm_miss==TRUE)/n()) # sum of everyone who has a missing value divided through by total number (n())
...
```{r}
gss %>% 
  summarize(prop_missing = sum(is.na(age_at_first_marriage))/n())
```
...
```{r}
gss %>% 
  select(worked_last_week) %>% 
  drop_na() %>% 
  summarize(prop_worked = sum(worked_last_week=="Yes")/n()) # sum of people who answered "Yes" divided by the total
```

```{r}
gss %>% 
  select(worked_last_week) %>% 
  mutate(is_wlw_miss = is.na(worked_last_week)) %>% # testing whether missing
  summarize(prop_missing = sum(is_wlw_miss==TRUE)/n())
```
...
```{r}
# for those with non-missing worked last week, counting and getting proportions by full_part_time_work
gss %>% 
  drop_na(worked_last_week) %>% 
  group_by(full_part_time_work) %>% 
  tally() %>% 
  mutate(prop = n/sum(n))
```
...
ggplot(data = gss, aes(age_at_first_marriage)) + 
  ...
  labs(x = "age at first marriage (years)", 
       title = "Age at first marriage, GSS")
```
...
# print
my_plot + ylab("Number of observations") + theme_light()
```
...
```{r}
ggplot(data = gss, aes(age_at_first_marriage)) + 
  geom_histogram(fill = "lightblue", color = "navy", binwidth = 1) + # bin width of 1 year
  ggtitle("Age at first marriage, GSS") + 
  xlab("age at first marriage (years)")

ggplot(data = gss, aes(age_at_first_marriage)) + 
  geom_histogram(fill = "lightblue", color = "navy", bins = 10)+ # plot a total of 10 bins/rectangles
  ggtitle("Age at first marriage, GSS") + 
  xlab("age at first marriage (years)")
```
...
```{r}
# add another variable, and vary the rectangle color (fill) by the respondent's sex
...
```{r}
# calculate our summary measure
resp_by_prov <- gss %>% 
  group_by(province) %>% 
  tally() %>% # counting the number of people by province
  mutate(prop = n / sum(n)) # getting the proportion
...
ggplot(data = resp_by_prov, aes(x = province, y = prop)) + 
  geom_bar(stat = "identity") + # plot what's in the dataframe 
  ylab("proportion")+
...
resp_by_prov <- resp_by_prov %>% 
  mutate(province = fct_reorder(province, prop)) # reorder the province factor/category by proportion 
```
...
ggplot(data = resp_by_prov, aes(x = province, y = prop)) + 
  ...
  ylab("proportion")+
  ggtitle("Proportion of GSS respondents by province") + 
  coord_flip() # flips to make horizontal 
```
...
```{r}
# getting my dataset that i want to plot
# only looking at 2017
# ordering region of world by the value of child mortality 
...
life_satis_age_sex <- gss %>% 
  group_by(age_group, sex) %>% # grouping by two variables 
...
ggplot(data = life_satis_age_sex, aes(x = age_group, y = mean_life_satis, colour = sex)) + 
  geom_point() + 
  geom_line() +
  scale_color_brewer(palette = "Set2") + # change the color scheme
...
```{r}
  ...
  geom_text() + # plotting the country code labels 
  ggtitle("TFR versus life expectancy, 2017")+
  theme_bw(base_size = 14)+ 
  ylab("life expectancy (years)") + 
  xlab("TFR (births per woman)") + 
...
```{r}
# make my dataset
country_ind_2014 <- country_ind %>% filter(year==2014)

ggplot(data = country_ind_2014, aes(x = gdp, y = life_expectancy, color = region)) + 
  geom_point() +
  labs(x = "GDP", y = "life expectancy (years)", title = "life expectancy v GDP, 2014")
...
prop_pb <- gss %>% 
  drop_na(place_birth_province) %>% # removing missing values
  group_by(place_birth_province) %>% 
  tally() %>% # count of respondents by province of birth
  mutate(prop = n/sum(n)) %>% # calculating proportion
  mutate(place_birth_province = fct_reorder(place_birth_province, prop)) # reordering

ggplot(data = prop_pb, aes(x = place_birth_province, y = prop, fill = place_birth_province)) + 
  geom_bar(stat = "identity") + 
  coord_flip() + 
  labs(y = "proportion of respondents born in province", 
       x = "province of birth",
       title = "GSS respondents by place of birth") + 
  theme_bw()+
