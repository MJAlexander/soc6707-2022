...
```{r}
# make a new binary variable called low_income
# if income_respondent is equal to < $25,000, low_income is equal to 1
# else, low_income is equal to 0
# Note: double equals
gss <- gss %>% 
  mutate(low_income = ifelse(income_respondent=="Less than $25,000", 1, 0))

gss <- gss %>% 
  mutate(low_income_lshs = ifelse(income_respondent=="Less than $25,000"&has_bachelor_or_higher=="no", 1, 0))
...
```{r}
# binary variable = 1 if income is <$25,000 OR income 25-49
gss <- gss %>% 
...
Look at mean age by income group
...
```
Plot by education: bar plots come in handy. Let's calculate the proportion of people who have low income by education group. 

```{r}
prop_df <- gss %>% 
  drop_na(has_bachelor_or_higher) %>% 
  group_by(has_bachelor_or_higher) %>% 
  summarize(prop_low_income = sum(low_income==1)/n()) # sum of people who have low income
# divided by the total number of people (n())


ggplot(data = prop_df, aes(x = has_bachelor_or_higher, y = prop_low_income)) + 
  geom_bar(stat = "identity")
```
Let's try this again, calculating and plotting proportion of respondents with low income by education and place of birth.

```{r}
prop_df <- gss %>% 
  drop_na(has_bachelor_or_higher, low_income, place_birth_canada) %>% # remove NAs
  group_by(has_bachelor_or_higher,place_birth_canada, low_income) %>% 
  tally() %>% # calculate counts by group
  group_by(has_bachelor_or_higher, place_birth_canada) %>% 
  # calculate proportion of people in each educ/pob category that have low income
  # also as a check doing high income (should sum to 1)
  summarize(prop_low_income = n[low_income==1]/sum(n), 
            prop_high_income = n[low_income==0]/sum(n), # CHECK, prop_low_income + prop_high_income = 1
            .groups = "drop") # this gets rid of the message, but don't need it
  


ggplot(data = prop_df, aes(x = place_birth_canada, y = prop_low_income, fill = has_bachelor_or_higher)) + 
