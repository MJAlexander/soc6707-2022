...
date: "Week 8: Logistic Regression"
output: 
  beamer_presentation:
    slide_level: 2
---
...
country_ind_2017 %>% 
  group_by(region) %>% 
  summarize(prop_countries = sum(high_tfr)/n(), 
            n_countries = n()) %>% 
  mutate(region = fct_reorder(region, prop_countries)) %>% 
  ggplot(aes(region, prop_countries)) + 
  geom_bar(stat = "identity")+coord_flip()+
  labs(title = "proportion of countries with high TFR", x = "", y = "proportion")
```
...
country_ind_2017 %>% 
  ...
  summarize(prop_countries = sum(high_tfr)/n(), 
            n_countries = n()) %>% 
  mutate(region = fct_reorder(region, -prop_countries)) %>% 
  ggplot(aes(region, prop_countries, size = n_countries)) + 
...
```{r}
country_ind_2017 %>% 
  ggplot(aes(life_expectancy, high_tfr))+geom_point()+
  labs(title = "high TFR versus life expectancy", y = "high TFR (yes = 1)", x = "life expectancy") 
```
...
- I.e. want to go from $y = \text{probabilities}$ to $y = \text{function(probabilities)}$ where the function lets $y$ be any real number.
...
$$
p =P\left(Y_{i}=1 \mid X_{i 1}, \ldots, X_{i k}\right)
$$
...
$$
\log \left(\frac{p}{1-p}\right)=\beta_{0}+\beta_{1} X_{i 1}+\cdots+\beta_{k} X_{i k} 
$$
where $\log \left(\frac{p}{1-p}\right)$ is known as the "log odds," or the "logit"
...
$$
p =\frac{\exp \left(\beta_{0}+\beta_{1} X_{i 1}+\cdots+\beta_{k} X_{i k}\right)}{1+\exp \left(\beta_{0}+\beta_{1} X_{i 1}+\cdots+\beta_{k} X_{i k}\right)}
$$
